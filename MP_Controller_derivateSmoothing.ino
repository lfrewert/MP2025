...
float Kp = 35.0;   // Proportional-Anteil: Reagiert auf aktuelle Abweichung
float Ki = 0.0;   // Integral-Anteil: Korrigiert bleibende Abweichung über Zeit
float Kd = 3.2;   // Differential-Anteil: Dämpft Oszillationen
...
void setup() {
  ...
  Serial.println("R/r = Debug-Modus an/aus");
  ...
  Serial.println("Befehl 'R' um Debug-Modus zu aktivieren.");
  Serial.println("---------------------------------------------------");
...
float computePID() {
  ...
  pidOutput = constrain(pidOutput, -200, 200);  // maxSpeed ist 255
...
void loop() {
  ...
  // ========================================
  // Ausgaben sind auskommentiert für maximale Performance
  // Mit Befehl 'R' über Serial aktivierbar
  // if (debugMode) {
  //   static unsigned long lastDiag = 0;
  //   if (millis() - lastDiag > 1000) {  // Nur 1x pro Sekunde
  ...
  if (Serial.available()) {
    ...
    // Debug-Modus umschalten
      ...
      debugMode = !debugMode;
      Serial.println(debugMode ? ">>> Debug-Modus AN" : ">>> Debug-Modus AUS");
    }
